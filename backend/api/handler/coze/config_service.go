...
// normalizeMoonshotKimiK26Connection keeps the K2.6-compatible OpenAI
// connection on Moonshot's China endpoint. In particular, the endpoint must
// be the API root rather than the chat-completions resource path.
func normalizeMoonshotKimiK26Connection(conn *config.Connection) (bool, error) {
	if conn == nil || conn.BaseConnInfo == nil || conn.BaseConnInfo.Model != moonshotKimiK26Model {
		return false, nil
	}

	if strings.TrimRight(conn.BaseConnInfo.BaseURL, "/") != moonshotKimiK26BaseURL {
		return true, fmt.Errorf("kimi-k2.6 requires base_url %q", moonshotKimiK26BaseURL)
	}
...
func CreateModel(ctx context.Context, c *app.RequestContext) {
	...
	isMoonshotKimiK26, err := normalizeMoonshotKimiK26Connection(req.Connection)
	if err != nil {
		invalidParamRequestResponse(c, err.Error())
		return
	}
	if isMoonshotKimiK26 {
		// K2.6 accepts image and video inputs only as data URLs or ms:// file
		// references. Enabling the transfer is required to prevent raw HTTP(S)
		// URLs from reaching the model.
		req.EnableBase64URL = true
	}
